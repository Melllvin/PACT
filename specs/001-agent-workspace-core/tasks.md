...
- [X] T069 [P] [US3] Tests de la grille : 1–4 agents → 2×2 avec « + » dans les emplacements libres, 5–6 → 3×2, ordre = `position`, couleurs inchangées au passage à 5 agents dans `tests/unit/renderer/tiles/TileGrid.test.tsx`
- [X] T070 [P] [US3] Tests de la tuile : bordure à la couleur de l'agent, ni numéro ni titre ni libellé d'état (FR-020), ⎇ au survol → branche actuelle + port, pulse à chaque changement d'état (FR-023), halo rouge fixe en `error`, actions selon l'état (Autoriser / Refuser en `awaiting-answer` ; Journal / Relancer / Reprendre en `error`) dans `tests/unit/renderer/tiles/Tile.test.tsx`
- [X] T071 [P] [US3] Test du terminal : l'instance xterm d'un agent est réutilisée (même objet) quand la tuile est démontée puis remontée, aucune perte de sortie dans `tests/unit/renderer/tiles/terminal-registry.test.ts`
- [X] T072 [US3] Tests d'intégration des actions avec le faux CLI : `agent:answer` écrit `answerKeys` et le scénario `ask-permission` reprend ; `crash-exit-1` → état `error` avec `lastError.code = 1` ; « Reprendre » relance avec `buildResume` (même `sessionId`) ; « Relancer » démarre une nouvelle session et retape la consigne initiale ; `agent:log` renvoie le tampon ; `rename-branch` → événement `agent:branch` ; `agent:close` avec et sans suppression du worktree (FR-037) dans `tests/integration/agents/agent-actions.test.ts`
- [X] T073 [P] [US3] Test e2e : 4 agents fake en 2×2, puis 6 en 3×2 ; autorisation via la tuile ; crash → halo + Reprendre ; terminal libre exécute `git status` à la racine du dépôt principal dans `tests/e2e/us3-tiles.spec.ts`

### Implementation for User Story 3

- [X] T074 [US3] Implémenter les actions agent (`answer`, `resume`, `restart`, `log`, `close`) dans `src/main/agents/agent-manager.ts` et les handlers `agent:answer`, `agent:resume`, `agent:restart`, `agent:close`, `agent:log`, `term:write`, `term:resize` dans `src/main/ipc/agent-handlers.ts`
- [X] T075 [P] [US3] Implémenter la surveillance de branche (`git branch --show-current` à chaque `Stop` et toutes les 5 s tant que l'agent tourne, événement `agent:branch`) dans `src/main/agents/branch-watcher.ts`
- [X] T076 [P] [US3] Implémenter la grille réactive avec emplacements « + » dans `src/renderer/tiles/TileGrid.tsx`
- [X] T077 [US3] Implémenter la tuile (bordure, ⎇ + infobulle, ⤢, actions en bas à droite, pulse CSS ponctuel, halo rouge fixe) en réutilisant `TerminalView` de T068 dans `src/renderer/tiles/Tile.tsx`, `src/renderer/tiles/TileActions.tsx`, `src/renderer/tiles/BranchTooltip.tsx`, `src/renderer/tiles/tile.module.css`
- [X] T078 [P] [US3] Implémenter le dialogue de fermeture d'agent (conserver / supprimer worktree et branche) et le panneau « Journal » dans `src/renderer/tiles/CloseAgentDialog.tsx` et `src/renderer/tiles/LogPanel.tsx`
- [X] T079 [US3] Afficher le terminal libre dans la grille et le brancher sur `FreeTerminal` dans `src/renderer/tiles/FreeTerminalTile.tsx`
...
- [X] T087 [P] [US5] Tests du Focus : ouverture par ⤢, en-tête branche + port, onglets Terminal (actif), Aperçu et Changements (inactifs), pastilles aux couleurs des agents, « ‹ Tuiles » et Échap hors du terminal ramènent à la grille, Échap dans le terminal est transmis au CLI dans `tests/unit/renderer/focus/FocusView.test.tsx`
...
- [X] T089 [P] [US5] Test e2e : ⤢ → Focus, saisie, pastille d'un autre agent → historique complet, Échap → grille, sorties intactes dans `tests/e2e/us5-focus.spec.ts`
...
- [X] T091 [US5] Implémenter le Focus et le sélecteur de pastilles en réutilisant `Tile` et `TerminalView` dans `src/renderer/focus/FocusView.tsx` et `src/renderer/focus/AgentPills.tsx`
- [X] T092 [US5] Ajouter l'action « Toujours pour ce worktree » à côté de Refuser / Autoriser en Focus dans `src/renderer/tiles/TileActions.tsx`
...
- [X] T111 [P] Tests puis implémentation des animations 1c (grille de points réactive autour du curseur, transition particules → tuiles → bordures, entrée dépliée des éléments À faire), désactivées si `prefers-reduced-motion` (FR-042), aucun effet dans les terminaux, dans `tests/unit/renderer/effects/effects.test.tsx`, `src/renderer/effects/DotGrid.tsx`, `src/renderer/effects/ViewTransition.tsx` — en partie couverte par T159 et T160 (R17) : points réactifs au curseur et au clic, entrée dépliée des éléments À faire, entrée échelonnée des vues, dans `src/renderer/effects/` ; les fichiers `DotGrid.tsx` et `ViewTransition.tsx` ne sont plus prévus. la transition particules → tuiles → bordures à chaque changement de vue est faite (`src/renderer/effects/transition.ts`, `--animate-tile-enter`), d'après la maquette « Grille réactive »
- [X] T112 Test e2e de performance SC-002 : 6 agents fake `burst-output` → latence d'écho d'une frappe < 100 ms (p95) et passage d'état visible dans À faire < 2 s (SC-003) dans `tests/e2e/perf.spec.ts`
- [X] T113 [P] Test e2e des chemins avec espaces et accents (dépôt dans « Mes Projets/Développement ») sur macOS et Windows dans `tests/e2e/paths.spec.ts`
...
- [X] T121 Tests puis implémentation : quand le dossier d'un workspace est supprimé ou déplacé, le workspace passe `unavailable` **et ses agents sont arrêtés proprement** (PTY tués, état persisté) dans `tests/integration/workspace/workspace-service.test.ts` et `src/main/workspace/workspace-service.ts` per spec Edge Cases (dossier supprimé) (missing)
- [X] T122 Tests puis implémentation de l'avertissement « modifications locales non incluses » affiché avant le lancement d'agents quand le dépôt a des modifications non commitées (réutilise `GitService.hasChanges`), dans `tests/unit/renderer/launch/QuickLaunch.test.tsx` et `src/renderer/launch/QuickLaunch.tsx` per spec Edge Cases (modifications non commitées) (partial)
- [X] T123 Tests puis implémentation du message d'accueil « aucun CLI détecté » expliquant comment installer Claude Code / Codex ou ajouter un CLI dans `tests/unit/renderer/home/Home.test.tsx` et `src/renderer/home/DetectedClis.tsx` per spec Edge Cases (aucun CLI détecté) (partial)
- [X] T124 Test e2e : un agent fake `burst-output` continue à produire pendant qu'un autre onglet workspace est actif ; au retour, aucune sortie perdue et l'état est à jour dans `tests/e2e/us3-background-tab.spec.ts` per FR-026 (partial)
- [X] T125 Exécuter la suite e2e smoke sur l'app packagée (`electron-builder --dir`) en CI macOS et Windows, en vérifiant le chargement de node-pty hors asar, via `tests/e2e/helpers/launch-app.ts` (option `PACT_E2E_PACKAGED=1`) et `.github/workflows/ci.yml` per plan R12 / R14 (partial)
...
## Phase 20: Migration shadcn/ui et effets React Bits (détail de T141, ajout manuel)

Décisions utilisateur du 2026-09-25 : une seule PR pour la phase 10 ; `src/renderer/effects/vendor/**` exclu de la couverture, du lint et du formatage ; suivre `docs/maquettes/Maquettes hi-fi.dc.html` (écrans du socle uniquement, « Hors socle » exclu). Rôles et noms accessibles inchangés : la migration ne change que le rendu.

- [X] T149 Fondations : polices Instrument Sans et JetBrains Mono embarquées (aucun chargement réseau, CSP), tokens 1c dans `@theme`, primitives shadcn nécessaires dans `src/renderer/components/ui/` (button, dialog, input, checkbox, radio-group, toggle-group, tooltip, badge), terminaux créés après le chargement de la police mono, dans `src/renderer/theme/` et `src/renderer/tiles/` — amendée (R17) : Geist remplace Instrument Sans ; primitives shadcn button, dialog, input, checkbox, radio-group, toggle, toggle-group, tooltip et badge ; les choix uniques (vues, pastilles du Focus, niveaux et portée des autorisations) sont des radios, décision de l'utilisateur du 2026-09-25
- [X] T150 Tests d'abord puis barre d'onglets et workspace vide (1b) : sélecteur « Tuiles | Comparer | Revue » (Comparer et Revue grisés), bouton « + Agents », bascule « À faire » avec compteur, zone vide pointillée « Ajouter des agents », dans `src/renderer/app/` et `src/renderer/workspace/` ; suppression de `shell.module.css` et `workspace.module.css`
- [X] T151 Tests d'abord puis accueil (1a) : titre « Ouvrir un workspace », recherche pleine largeur, sections « Déjà ouverts » / « Récents » en cartes, colonne droite « Autre dépôt » (dépôt de dossier, choix, clonage) et « Agents détectés », dans `src/renderer/home/` ; suppression de `home.module.css`
- [X] T152 Tests d'abord puis menu rapide (1c), autorisations (1m, case « Reprendre automatiquement » conservée per FR-035) et mode détaillé (1d, maître-détail) en Dialog shadcn (Entrée = choix par défaut conservé), dans `src/renderer/launch/` ; suppression de `launch.module.css`
- [X] T153 Tests d'abord puis tuiles (1e/1f/1l/1n) et Focus (1p) : bordure couleur de l'agent, actions pleines en bas à droite (✓ Autoriser, ✕ Refuser, Reprendre…), icônes ⎇ / ⤢ / fermer en haut à droite avec infobulle de branche, halo rouge d'erreur, état toujours exposé aux technologies d'assistance (FR-041), « reprise auto à HH:MM · Annuler » (FR-036), dans `src/renderer/tiles/` et `src/renderer/focus/` ; suppression de `tiles.module.css`
...
Décisions utilisateur du 2026-09-25 : suivre `docs/maquettes/Maquette interactive.dc.html` pour
