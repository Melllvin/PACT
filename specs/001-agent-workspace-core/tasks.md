...
- [ ] T149 Fondations : polices Instrument Sans et JetBrains Mono embarquées (aucun chargement réseau, CSP), tokens 1c dans `@theme`, primitives shadcn nécessaires dans `src/renderer/components/ui/` (button, dialog, input, checkbox, radio-group, toggle-group, tooltip, badge), terminaux créés après le chargement de la police mono, dans `src/renderer/theme/` et `src/renderer/tiles/` — amendée (R17) : Geist remplace Instrument Sans ; polices, tokens, button, dialog, input et attente de la police des terminaux faits. Reste à décider avec l'utilisateur : checkbox, radio-group, toggle-group, tooltip et badge ne sont pas des primitives shadcn, ils sont faits main (infobulle de `BranchTooltip`, sélecteur Vues et bascules des autorisations en boutons `aria-pressed`, radios natives stylées des dialogues de fermeture et d'autorisation, pastille À faire) ; les migrer changerait des rôles testés
