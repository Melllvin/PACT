...
- [ ] T149 Fondations : polices Instrument Sans et JetBrains Mono embarquées (aucun chargement réseau, CSP), tokens 1c dans `@theme`, primitives shadcn nécessaires dans `src/renderer/components/ui/` (button, dialog, input, checkbox, radio-group, toggle-group, tooltip, badge), terminaux créés après le chargement de la police mono, dans `src/renderer/theme/` et `src/renderer/tiles/`
- [ ] T150 Tests d'abord puis barre d'onglets et workspace vide (1b) : sélecteur « Tuiles | Comparer | Revue » (Comparer et Revue grisés), bouton « + Agents », bascule « À faire » avec compteur, zone vide pointillée « Ajouter des agents », dans `src/renderer/app/` et `src/renderer/workspace/` ; suppression de `shell.module.css` et `workspace.module.css`
- [ ] T151 Tests d'abord puis accueil (1a) : titre « Ouvrir un workspace », recherche pleine largeur, sections « Déjà ouverts » / « Récents » en cartes, colonne droite « Autre dépôt » (dépôt de dossier, choix, clonage) et « Agents détectés », dans `src/renderer/home/` ; suppression de `home.module.css`
- [ ] T152 Tests d'abord puis menu rapide (1c), autorisations (1m, case « Reprendre automatiquement » conservée per FR-035) et mode détaillé (1d, maître-détail) en Dialog shadcn (Entrée = choix par défaut conservé), dans `src/renderer/launch/` ; suppression de `launch.module.css`
- [ ] T153 Tests d'abord puis tuiles (1e/1f/1l/1n) et Focus (1p) : bordure couleur de l'agent, actions pleines en bas à droite (✓ Autoriser, ✕ Refuser, Reprendre…), icônes ⎇ / ⤢ / fermer en haut à droite avec infobulle de branche, halo rouge d'erreur, état toujours exposé aux technologies d'assistance (FR-041), « reprise auto à HH:MM · Annuler » (FR-036), dans `src/renderer/tiles/` et `src/renderer/focus/` ; suppression de `tiles.module.css`
- [ ] T154 Tests d'abord puis colonne À faire en cartes (pastille, libellé court, titre, commande ou « tapez dans le terminal », carte de réponse bordée orange), dans `src/renderer/todo/` ; suppression de `todo.module.css`
- [ ] T155 Tests d'abord puis dialogues « Fermer l'agent », « Journal » et « Ajouter un CLI » en Dialog shadcn, test « Dialog shadcn ouvert sans erreur console » (reste de T142), suppression de `use-dialog-keys`
- [ ] T156 Tests d'abord puis grille de points React Bits `DotGrid` copiée dans `src/renderer/effects/vendor/`, enrobée dans `src/renderer/effects/` : tokens de la grille réactive 1c, rien sous `prefers-reduced-motion`, sans `matchMedia` ou sans canvas, jamais sous un terminal (FR-042, FR-020), boucle à l'arrêt quand le pointeur ne bouge plus
