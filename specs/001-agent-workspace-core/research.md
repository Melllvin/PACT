...
- **Decision** : un **adaptateur par CLI** (contrat `contracts/cli-adapter.md`) qui fournit les
  ...
  1. **Hooks natifs du CLI** relayés vers PACT :
     ...
     - Codex (≥ versions récentes, dernière 0.156.1) supporte des hooks (`features.hooks = true`,
       ...
       Version : Codex 0.156.1 (installé le 2026-09-23) expose `hooks` comme fonctionnalité
       stable, activée par défaut (`codex features list`). La détection vérifie la présence de ce
       flag ; sinon le CLI est marqué `unsupported-version` et l'accueil invite à le mettre à jour.
       **Confiance des hooks** : Codex n'exécute que des hooks dont la confiance a été enregistrée ;
       ...
       hooks PACT une fois. Mécanisme d'approbation exact à caractériser dans la première tâche
       de l'adaptateur Codex.
...
- **Codex** : l'identifiant est lu dans l'événement `SessionStart` ; « Reprendre » =
